...
/*
 * Constellations (FILE / arbitrary read CTF)
 *
 * - Player can choose a map size: 8, 16, 32, or 64.
 * - Internally, we always keep a 64x64 backing canvas in .bss.
 * - We also snapshot a real FILE object into .bss (snapshot_file).
 * - A bulk "raw import" feature writes load_cells bytes starting at canvas.
 * - BUG: load_cells is only updated when the map size grows, not when it shrinks.
 *   So if you go 64 -> 8, bulk import still writes as if the map were 64x64,
 *   overflowing into snapshot_file.
 * - The overflow is clamped to only cover the first few fields of the FILE
 *   (_flags and _IO_read_{ptr,end,base}), giving a partial overwrite.
 * - A "telescope" view interprets those read-buffer pointers as an address
 *   range and visualizes the bytes as a 16x16 nibble constellation.
 * - There's also a format-string bug on the constellation name for leaks.
 */
...
/* Snapshot of a real FILE object; never used with stdio after snapshot */
static FILE snapshot_file;
static FILE *loaded_fp = NULL;      /* currently opened constellation file */
...
/* Game state */
static size_t load_cells;     /* how many bytes bulk import writes (BUG: stale when shrinking) */


/* Brightness lookup for leak visualization */
static const char BRIGHTNESS_LUT[] = " .:-=+*#%@";
/* Brightness lookup for stacked stars on the main canvas */
...
/* Backing canvas (heap-allocated per current map size) */
struct eight_by_eight *canvas = NULL;
size_t current_size;   /* logical map size: 8,16,32,64 */
...
/* List constellation files; if should_print, display them with 1-based indices.
 * Returns the number of entries stored in entries[][].
 */
static size_t list_constellations(char entries[][256], size_t max_entries, int should_print) {
...
static struct eight_by_eight *load_constellation_points(FILE *fp, char *name) {
    ...
    /* Reset to a clean 64x64 canvas before loading (forces 64x64 view). */
    current_size = 32;
    struct eight_by_eight *temp_canvas = alloc_canvas();
    load_cells = current_size * current_size;
...
static void banner(void) {
    ...
    #ifdef DEBUG
    puts("  i  - info / debug (addresses, sizes)");
    #endif
    puts("  c  - clear map");
    puts("  q  - quit");
    puts("=======================================");
...
static void plot_point(void) {
    ...
    size_t off = (size_t)y * current_size + (size_t)x;

    #ifdef DEBUG
    printf("[debug] plot_point: off = %zu, ch = %c\n", off, ch);
    printf("[debug] canvas: %p\n", canvas);
    printf("[debug] canvas->pixels: %p\n", canvas->pixels);
    printf("[debug] y-offset: %zu\n", y);
    printf("[debug] x-offset: %zu\n", x);
    printf("[debug] canvas->pixels[off]: %p\n", &(canvas->pixels[off]));
    #endif
...
static void set_map_size(void) {
    ...
    #ifdef DEBUG
    printf("[*] Bulk import will currently read %zu bytes.\n", load_cells);
    printf("[*] canvas: %p\n", canvas);
    #endif
}
...
static void info(void) {
    printf("[info] current_size = %zu\n", current_size);
    printf("[info] load_cells   = %zu (bytes bulk import will try to read)\n", load_cells);
    printf("[info] canvas       @ %p (bytes=%zu, side=%zu)\n",
           (void*)canvas, current_size * current_size, current_size);
    printf("[info] canvas_ptr   = %p\n", (void*)canvas);
    printf("[info] snapshot_file@ %p (FILE snapshot in .bss)\n", (void*)&snapshot_file);
}
#endif
...
int main(void) {
    /* Synchronous I/O for exploit dev determinism */
    ...
    for (;;) {
        ...
        switch (cmd[0]) {
            ...
            case 'q': case 'Q':
                ...
                return 0;
            default:
                puts("[!] Unknown command. Try: m v p n s f i c q");
                break;
        }
    }
